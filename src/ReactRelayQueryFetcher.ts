...
import { ExecuteConfig, FetchOptions } from 'react-relay/lib/ReactRelayQueryFetcher';
...
class ReactRelayQueryFetcher {
  ...
  execute({
    ...
    const reference = environment.retain(operation.root, true);
  ...
  setOnDataChange(onDataChange: any): void {
    invariant(
      this._fetchOptions,
  ...
  _retainCachedOperation(
    ...
    this._cacheSelectionReference = environment.retain(operation.root, false);
  }
